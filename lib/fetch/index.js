...
import { createUnzip } from 'zlib';   // 压缩解压模块
...
const MAX_REDIRECTS = 5,              // 最大重定向次数常量
    ...
    // 处理请求体函数
    _processRequestBody = (options, url, fetchRes, finishedFlag) => {
    ...
    // 处理重定向函数
    _handleRedirect = (res, url, options, fetchRes, finishedFlag, req) => {
     ...
     * @param {Cookies|Array|string} [options.cookies] - Cookie 管理器实例、Cookie 字符串数组或单个 Cookie 字符串,
     * 用于自动管理请求和响应的 Cookie;
    ...
    nmfetch = (url, options = {}) => {
        ...
        // 处理HTTPS协议的SNI
        if (protocol === 'https:' && host && !isIP(host) && !reqOptions.servername) reqOptions.servername = host;
