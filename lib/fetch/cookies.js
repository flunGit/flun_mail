...
class Cookies {
    ...
    set(cookieStr, url) {
        ...
        cookie.domain = !domain || !this.#isDomainMatch(hostname, domain) ? hostname : domain;
    ...
    match(cookie, url) {
        ...
        if (!this.#isDomainMatch(urlObj.hostname, cookie.domain)) return false; // 检查主机名是否不匹配Cookie域名
