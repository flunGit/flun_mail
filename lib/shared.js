...
// 引入必要的Node.js内置模块
const util = require('util'), fs = require('fs'), dns = require('dns'), NET = require('net'), OS = require('os'),
    PK = (({ name, version, homepage }) => ({ name, version, homepage }))(require('../package.json')),
    wellKnown = require('./well-known'), nmfetch = require('./fetch'), { regexs, resetRegex } = require('./regexs'),

    DNS_TTL = 5 * 60 * 1000, DNS_MAP = new Map(); // DNS缓存生存时间(5分钟)和DNS缓存Map
