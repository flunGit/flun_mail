...
import { createReadStream as fsReadStream } from 'fs';
import { isIP, connect as netConnect } from 'net';
...
import { regexs, resetRegex } from './regexs.js';
import pkg from '../package.json' with { type: 'json' };

const PK = (({ name, version, homepage }) => ({ name, version, homepage }))(pkg),
    DNS_TTL = 5 * 60 * 1000, DNS_MAP = new Map(), dnsLookup = dns.lookup, dnsResolve = dns.resolve,
     ...
     */
    _isFamilySupported = (family, allowInternal) => {
        try {
            const netInter = networkInterfaces(); // 获取网络接口信息
            // 如果没有网络接口信息,支持所有地址族;否则,筛选出指定地址族的网络接口
            return !netInter ? true : Object.values(netInter).flat()
     ...
     */
    _resolver = (family, hostname, options = {}, callback) => {
        // 检查地址族是否支持
     ...
     */
    _handleReadableStream = (stream, chunkArr, lenRef) => {
        let chunk;
    ...
    _createDefaultLogger = levels => {
        ...
        const levelMaxLen = Math.max(...levels.map(l => l.length)),
            ...
            print = (level, entry, message, ...args) => {
                const prefix = buildPrefix(entry), formattedMessage = format(message, ...args),// 构建前缀,格式化消息
    ...
    resolveHostname = (options = {}, callback) => {
        ...
        _resolver(4, newHost, options, (err, addrV4) => {
            ...
            _resolver(6, newHost, options, (err, addrV6) => {
                ...
                try {
                    dnsLookup(newHost, { all: true }, (err, lookupAddr) => {
                        if (err) return handleError(err);
     ...
     */
    resolveContent = (data, key, callback) => {
        // 如果没有提供回调，创建Promise
        ...
        if (typeof content === 'object') {
            ...
            // 处理文件路径
            else if (path) return resolveStream(fsReadStream(path), callback);
        }
     ...
     */
    initSmtpConstructor = (instance, options, SmtpC, defaultComponent) => {
     ...
     */
    createSmtpConnection = (transport, socketOptions, SmtpC) => {
     ...
     * @returns {Object} 清理函数
     */
    setupConnectionHandlers = (state, connection, callback, transport) => {
     ...
     */
    performSmtpAuthentication = (state, connection, auth, options, callback, transport, onSuccess) => {
        if (state.returned) return;
     ...
     */
    createAuthConfig = (authData, logger, mailer = null, OAUTH2) => {
        if (!authData) return '';

        let hasAuth = false;
        const authConfig = {}, mergeAuthData = source => {
     ...
     */
    prepareMessageForSending = (mail, logger, connectionId = '', options = {}, done) => {
        const { data, message, normalize } = mail, { envelope: En, dsn, _dkim, ses } = data, { _headers } = message;
        ...
        if (options.transportType === 'json')
            ...
            // 构建sendmail参数
            const { sArgs } = options;
            args = sArgs ? ['-i', ...sArgs, ...to] : ['-i', ...(from ? ['-f', from] : []), ...to];
     ...
     * @param {Object} logger 日志记录器
     */
    handleSendResult = (result, callback, logger) => {
     ...
     * @param {Object} auth 认证信息
     */
    verifySmtp = (transport, callback, SmtpC, auth = null) => {
        // 如果没有提供回调，则创建Promise
        const promise = !callback ? new Promise((resolve, reject) => callback = callbackPromise(resolve, reject)) : null;
...
export {
    PK, format, isIP, netConnect, dnsLookup, dnsResolve, fsReadStream, osHostname, nmfetch, regexs, resetRegex, newURL,
