...
import { XOAuth2 } from '../xoauth2.js';
import { SmtpConnection } from '../smtp-connection/index.js';
import {
    createSmtpConnection, setupConnectionHandlers, performSmtpAuthentication, prepareMessageForSending,
    handleSendResult, createAuthConfig
} from '../shared.js';
...
class PoolResource extends EventEmitter {
    ...
    getAuth(authOpts) {
        ...
        return createAuthConfig(authData, this.logger, this.mailer, XOAuth2);           // 创建认证配置
    }
    ...
    send(mail, callback) {
        ...
        this.connection.send(envelope, readStream, (err, info) => {
            ...
            setImmediate(() => {
                ...
                // 检查速率限制并标记为可用
                else this.pool.checkRateLimit(() => this.available = true, this.emit('available'));
            });
