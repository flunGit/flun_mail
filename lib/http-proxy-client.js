 ...
 *
 * 使用示例：
 * httpProxyClient("http://localhost:3128/", 80, "google.com", (err, socket)=>{
 ...
 */
const httpProxyClient = (proxyUrl, destinationPort, destinationHost, callback) => {
    let proxy;
    try {
    ...
    const { hostname, port, protocol, username = '', password = '' } = proxy, isHttps = protocol === 'https:',
        ...
        connect = isHttps ? (options, callback) => tlsConnect(options, callback)
            : (options, callback) => netConnect(options, callback), // 根据代理协议类型选择连接方式
        ...
        socket = connect(options, () => {
            ...
            const onSocketData = chunk => {
                ...
                const headers = Buffer.concat(headerChunks).toString('latin1'), headersEndIndex = headers.indexOf('\r\n\r\n');
                if (headersEndIndex !== -1) {
                    ...
                    if (remainder) socket.unshift(Buffer.from(remainder, 'latin1'));
                    state.finished = true;
