...
import {
    PK, osHostname, isIP, netConnect, regexs, resetRegex, getLogger, resolveHostname, callbackPromise, encodeXText, cleanup
} from '../shared.js';

// 默认超时(连接,套接字无活动,等待SMTP问候,DNS解析)时间（毫秒）
const CONNECTION_TIMEOUT = 2 * 60 * 1000, SOCKET_TIMEOUT = 10 * 60 * 1000,
    GREETING_TIMEOUT = 30 * 1000, DNS_TIMEOUT = 30 * 1000;
...
class SmtpConnection extends EventEmitter {
    constructor(options = {}) {
        ...
        this.servername = servername ? servername : !isIP(this.host) ? this.host : false;
        ...
        this.name = name || this.#getHostname();
         ...
         * 套接字监听器的回调函数
         */
        this.onSocketData = chunk => this.#onData(chunk), this._onSocketClose = () => this.#onClose();
        this.onSocketError = error => this.#onError(error, 'ESOCKET', false, 'CONN');
        this.onSocketEnd = () => this.#onEnd(), this.onSocketTimeout = () => this.#onTimeout();
    }
    ...
    #setupConnectionHandlers() {
        this._connectionTimeout = setTimeout(() => this.#onError('连接超时', 'ETIMEDOUT', false, 'CONN'),
     ...
     * 处理 DNS 解析和连接建立
     */
    #resolveAndConnect(opts, connectCallback) {
        resolveHostname(opts, (err, resolved) => {
            if (err) return setImmediate(() => this.#onError(err, 'EDNS', false, 'CONN'));

            this.#logDnsResolution(opts.host, resolved);
            Object.keys(resolved).forEach(key => {
                if (!key.startsWith('_') && resolved[key]) opts[key] = resolved[key];
            });

            try {
                connectCallback(opts), this.#setupConnectionHandlers();
            } catch (E) {
                return setImmediate(() => this.#onError(E, 'ECONNECTION', false, 'CONN'));
            }
        });
    }
     ...
     * 记录 DNS 解析日志
     */
    #logDnsResolution(source, resolved) {
        this.logger.debug(
    ...
    connect(connectCallback) {
        if (typeof connectCallback === 'function') {
            ...
            const destroyedMsg = this.#isDestroyedMessage('connect');
            if (destroyedMsg) return connectCallback(this.#formatError(destroyedMsg, 'ECONNECTION', false, 'CONN'));
        }
        ...
        // 如果已提供连接对象，则使用它
        if (connection) {
            this._socket = connection, this.#setupConnectionHandlers();
            if (this.secureConnection && !this.alreadySecured)
                setImmediate(() =>
                    this.#upgradeConnection(err => {
                        if (err) return this.#onError(new Error(`启动TLS错误-${(err.message || err)}`), 'ETLS', false, 'CONN');
                        this.#onConnect();
                    })
                );
            else setImmediate(() => this.#onConnect());
        ...
        const createConnection = connectionCreator => {
            return this.#resolveAndConnect(opts, resolvedOpts => {
                this._socket = connectionCreator(resolvedOpts);
                this._socket.setKeepAlive(true), this._socket.once('connect', () => this.#onConnect());
            });
        };
        // 如果套接字已设置，则使用它
        if (socket)
        ...
        // 如果secureConnection为true，则使用TLS连接
            ...
            return createConnection((resolvedOpts) => tlsConnect(resolvedOpts));
        }
        return createConnection((resolvedOpts) => netConnect(resolvedOpts));           // 否则,则使用明文连接
    }
     ...
     */
    quit() {
        this.#sendCommand('QUIT'), this._responseActions.push(this.close);
    }
    ...
    * 生成AUTH PLAIN认证字符串
    */
    #generateAuthPlainString(includePassword = true) {
    ...
    login(authData = {}, callback) {
        const destroyedMsg = this.#isDestroyedMessage('login');
        if (destroyedMsg) return callback(this.#formatError(destroyedMsg, 'ECONNECTION', false, 'API'));
        ...
        if (this._authMethod !== 'XOAUTH2' && (!credentials || !credentials.user || !credentials.pass)) {
            ...
            else return callback(this.#formatError(`缺少 "${this._authMethod}" 的凭据`, 'EAUTH', false, 'API'));
        }
        ...
        if (this.customAuth.has(this._authMethod)) {
            ...
            const handler = this.customAuth.get(this._authMethod), state = { returned: false },
                ...
                reject = err => {
                    cleanup(state), callback(this.#formatError(err, 'EAUTH', lastResponse, `AUTH ${this._authMethod}`));
                },
                ...
                handlerResponse = handler({
                    ...
                    sendCommand: (cmd, done) => {
                        ...
                        setImmediate(() => this.#sendCommand(cmd));
                        return promise;
                    }, resolve, reject
                });
        ...
        switch (this._authMethod) {
            case 'XOAUTH2':
                return this.#actionAUTH_XOAUTH2_TOKEN(false, callback);
            case 'LOGIN':
                this._responseActions.push(str => this.#actionAUTH_LOGIN_USER(str, callback)), this.#sendCommand('AUTH LOGIN');
                return;
            case 'PLAIN':
                this._responseActions.push(str => this.#actionAUTH_PLAIN_RESPONSE(str, callback));
                this.#sendCommand(`AUTH PLAIN ${this.#generateAuthPlainString(true)}`,// 认证参数
                    `AUTH PLAIN ${this.#generateAuthPlainString(false)}`);            // 日志参数(不包含密码)
                return;
            case 'CRAM-MD5':
                this._responseActions.push(str => this.#actionAUTH_CRAM_MD5(str, callback));
                this.#sendCommand('AUTH CRAM-MD5');
                return;
        }

        return callback(this.#formatError(`未知的认证方法 "${this._authMethod}"`, 'EAUTH', false, 'API'));
    }
    ...
    send(envelope, message, done) {
        if (!message) return done(this.#formatError('空消息', 'EMESSAGE', false, 'API'));

        const destroyedMsg = this.#isDestroyedMessage('send message');
        if (destroyedMsg) return done(this.#formatError(destroyedMsg, 'ECONNECTION', false, 'API'));

        // 拒绝大于允许大小的消息
        if (this._maxAllowedSize && envelope.size > this._maxAllowedSize)
            return setImmediate(() => {
                done(this.#formatError(`消息大小超过允许值 ${this._maxAllowedSize}`, 'EMESSAGE', false, 'MAIL FROM'));
            });

        const state = { returned: false }, callback = (...args) => { cleanup(state), done(...args) };
        if (typeof message.on === 'function') message.on('error', err => callback(this.#formatError(err, 'ESTREAM', false, 'API')));

        // 将消息写入流
        const writeMessageToStream = stream => {
            typeof message.pipe === 'function' ? message.pipe(stream) : (stream.write(message), stream.end());
        }, startTime = Date.now();
        this.#setEnvelope(envelope, (err, info) => {
            if (err) {
            ...
            const envelopeTime = Date.now(),
                stream = this.#createSendStream((err, str) => {
    ...
    reset(callback) {
        ...
        this._responseActions.push(str => {
            if (!str.startsWith('2')) return callback(this.#formatError(`无法重置会话状态;响应=${str}`, 'EPROTOCOL', str, 'RSET'));
    ...
    #onConnect() {
        ...
        // 清除套接字的现有监听器
        this._socket.removeListener('data', this.onSocketData).removeListener('timeout', this.onSocketTimeout)
            .removeListener('close', this._onSocketClose).removeListener('end', this.onSocketEnd);
        // 设置新监听器
        this._socket.on('data', this.onSocketData).once('close', this._onSocketClose).once('end', this.onSocketEnd)
            .setTimeout(this.options.socketTimeout || SOCKET_TIMEOUT).on('timeout', this.onSocketTimeout);

        this._greetingTimeout = setTimeout(() => {
            // 如果仍在等待问候消息，则放弃
            if (this._socket && !this._destroyed && this._responseActions[0] === this.#actionGreeting)
                this.#onError('从未收到问候消息', 'ETIMEDOUT', false, 'CONN');

        }, this.options.greetingTimeout || GREETING_TIMEOUT);

        this._responseActions.push(this.#actionGreeting), this._socket.resume();
    }
    ...
    #onClose() {
        ...
        if (this.upgrading && !this._destroyed) return this.#onError(new Error('连接意外关闭'), 'ETLS', serverResponse, 'CONN');
        else if ((![this.#actionGreeting, this.close].includes(this._responseActions[0]) && !this._destroyed) ||
            regexs.RESPONSE_4XX_5XX.test(serverResponse))
            return this.#onError(new Error('连接意外关闭'), 'ECONNECTION', serverResponse, 'CONN');
     ...
     */
    #onTimeout() {
        return this.#onError(new Error('超时'), 'ETIMEDOUT', false, 'CONN');
    }
    ...
    #upgradeConnection(callback) {
        ...
        this.upgrading = true;
        try {
            const tlsSocket = tlsConnect(opts, () => {
                this.secure = true, this.upgrading = false, tlsSocket.on('data', this.onSocketData);
                // 清理旧socket的事件剩余监听器
                socketPlain.removeListener('error', this.onSocketError).removeListener('close', this._onSocketClose)
                    .removeListener('end', this.onSocketEnd);
                return callback(null, true);
            });
            // 更新实例socket引用,并绑定新socket的事件处理器
            this._socket = tlsSocket;
            tlsSocket.setTimeout(this.options.socketTimeout || SOCKET_TIMEOUT).on('timeout', this.onSocketTimeout)
                .on('error', this.onSocketError).once('close', this._onSocketClose).once('end', this.onSocketEnd);
        } catch (err) {
    ...
    #processResponse() {
        ...
        if (!str.trim()) setImmediate(() => this.#processResponse());        // 跳过意外的空行

        const action = this._responseActions.shift();
        if (typeof action === 'function') action.call(this, str), setImmediate(() => this.#processResponse());
        else return this.#onError(new Error('意外响应'), 'EPROTOCOL', str, 'CONN');
    }
     ...
     */
    #setEnvelope(envelope = {}, callback) {
        const args = [], errors = [];
        this._envelope = envelope;
        let { from = '', to, dsn, use8BitMime, size } = this._envelope, useSmtpUtf8 = false;
        // 处理发件人地址
        from = ((from?.address) || from).toString().trim();
        if (!from) return callback(this.#formatError('发件人地址不能为空', 'EENVELOPE', false, 'API'));
        // 处理收件人地址
        to = [].concat(to || []).map(to => ((to?.address) || to || '').toString().trim()).filter(ad => ad);
        if (!to.length) return callback(this.#formatError('未定义收件人', 'EENVELOPE', false, 'API'));
        to = [...new Set(to)]; // 去重
        // 验证单个地址并检测非ASCII字符
        ...
        if (errors.length > 0) return callback(this.#formatError(errors.join('; '), 'EENVELOPE', false, 'API'));
        ...
        if (dsn) {
            try {
                dsn = this.#setDsnEnvelope(dsn);
            } catch (err) {
                return callback(this.#formatError(`无效的DSN:${err.message}`, 'EENVELOPE', false, 'API'));
            }
        }

        this._responseActions.push(str => this.#actionMAIL(str, callback));
        ...
        this.#sendCommand(`MAIL FROM:<${from}>${args.length ? ' ' + args.join(' ') : ''}`);
    }

    // 设置DSN信封属性
    #setDsnEnvelope(params) {
    ...
    #createSendStream(callback) {
        ...
        if (this.options.lmtp)
            accepted.forEach((recipient, i) => {
                rActions.push(str => this.#actionLMTPStream(recipient, i === lastIndex, str, callback));
            });
        else rActions.push(str => this.#actionSMTPStream(str, callback));
    ...
    #sendHeloCommand() {
        const [action, command] = this.options.lmtp ? [this.#actionLHLO, 'LHLO'] : [this.#actionEHLO, 'EHLO'];
        this._responseActions.push(action), this.#sendCommand(`${command} ${this.name}`);
    }
    ...
    #actionGreeting(str) {
        ...
        if (!str.startsWith('220')) return this.#onError(new Error(`无效的问候;响应=${str}`), 'EPROTOCOL', str, 'CONN');
        this.#sendHeloCommand();
    }
     ...
     */
    #actionLHLO(str) {
        if (!str.startsWith('2')) return this.#onError(new Error(`无效的LHLO;响应=${str}`), 'EPROTOCOL', str, 'LHLO');
        this.#actionEHLO(str);
    }
     ...
     */
    #actionEHLO(str) {
        if (str.startsWith('421')) return this.#onError(new Error(`服务器终止连接;响应=${str}`), 'ECONNECTION', str, 'EHLO');

        // 通用命令队列函数
        const queueCommand = (action, command) => {
            this._responseActions.push(action), this.#sendCommand(command);
        }, { requireTLS, ignoreTLS } = this.options;
        if (!str.startsWith('2')) {
            if (requireTLS)
                return this.#onError(new Error(`EHLO失败但HELO不支持必需的STARTTLS;响应=${str}`), 'ECONNECTION', str, 'EHLO');
            return queueCommand(this.#actionHELO, `HELO ${this.name}`);  // 使用通用函数发送 HELO
        }
        ...
        if (!this.secure && !ignoreTLS && (regexs.STARTTLS.test(str) || requireTLS))
            return queueCommand(this.#actionSTARTTLS, 'STARTTLS'); // 使用通用函数发送 STARTTLS
     ...
     */
    #actionHELO(str) {
        if (!str.startsWith('2')) return this.#onError(new Error(`无效的HELO;响应=${str}`), 'EPROTOCOL', str, 'HELO');
    ...
    #actionSTARTTLS(str) {
        if (!str.startsWith('2')) {
            ...
            return this.#onError(new Error('使用STARTTLS升级连接时出错'), 'ETLS', str, 'STARTTLS');
        }

        this.#upgradeConnection((err, secured) => {
            if (err) return this.#onError(new Error(`启动TLS错误-${err.message || err}`), 'ETLS', false, 'STARTTLS');

            this.logger.info({ tnx: 'smtp' }, '连接已使用STARTTLS升级');
            secured ? this.#sendHeloCommand() : this.emit('connect');// 如果升级成功，重新启动会话,否则继续未加密
        });
    }
     ...
     */
    #actionAUTH_LOGIN_USER(str, callback) {
        if (!regexs.AUTH_RESPONSE.test(str))
            return callback(this.#formatError('在等待"334 VXNlcm5hbWU6"时登录序列无效', 'EAUTH', str, 'AUTH LOGIN'));

        this._responseActions.push(str => this.#actionAUTH_LOGIN_PASS(str, callback));
        this.#sendCommand(Buffer.from(this._auth.credentials.user, 'utf-8').toString('base64'));
    }
     ...
     */
    #actionAUTH_CRAM_MD5(str, callback) {
        const challengeMatch = str.match(regexs.CRAM_MD5_CHALLENGE);
        let challengeString = '';
        if (!challengeMatch) return callback(this.#formatError('在等待服务器响应时登录序列无效', 'EAUTH', str, 'AUTH CRAM-MD5'));
        ...
        this._responseActions.push(str => this.#actionAUTH_CRAM_MD5_PASS(str, callback));
        this.#sendCommand(Buffer.from(prepended).toString('base64'), Buffer.from(`${user} /* secret */`).toString('base64'));
    }
     ...
     * @param {String} str 来自服务器的消息
     */
    #actionAUTH_CRAM_MD5_PASS(str, callback) {
        if (!str.match(regexs.AUTH_SUCCESS))
            return callback(this.#formatError('在等待"235"时登录序列无效', 'EAUTH', str, 'AUTH CRAM-MD5'));
     ...
     * @param {String} str 来自服务器的消息
     */
    #actionAUTH_LOGIN_PASS(str, callback) {
        if (!regexs.AUTH_RESPONSE.test(str))
            return callback(this.#formatError('在等待"334 UGFzc3dvcmQ6"时登录序列无效', 'EAUTH', str, 'AUTH LOGIN'));
     ...
     * @param {String} str 来自服务器的消息
     */
    #actionAUTH_PLAIN_RESPONSE(str, isRetry, callback) {
        ...
        if (str.startsWith('334')) {
            this._responseActions.push(str => {
                if (isRetry || method !== 'XOAUTH2') this.#actionAUTH_PLAIN_RESPONSE(str, true, callback);
                else setImmediate(() => this.#actionAUTH_XOAUTH2_TOKEN(true, callback));  // 获取新的OAuth2访问令牌
            });
            this.#sendCommand('');
            return;
        }
        ...
        if (!str.startsWith('2')) {
            ...
            return callback(this.#formatError('无效的登录', 'EAUTH', str, `AUTH ${method}`));
        }
     ...
     * @param {String} str 来自服务器的消息
     */
    #actionMAIL(str, callback) {
        if (!str.startsWith('2')) {
            const message = this._usingSmtpUtf8 && str.startsWith('550 ') && regexs.NON_ASCII.test(this._envelope.from) ?
                '不允许国际化的邮箱名称' : '邮件命令失败';
            return callback(this.#formatError(message, 'EENVELOPE', str, 'MAIL FROM'));
        }

        const { rcptQueue } = this._envelope;
        if (!rcptQueue.length) return callback(this.#formatError("无法发送邮件-未定义收件人", 'EENVELOPE', false, 'API'));

        this._recipientQueue = [];
        const processRecipient = (recipient) => {
            this._recipientQueue.push(recipient), this._responseActions.push(str => this.#actionRCPT(str, callback));
            this.#sendCommand(`RCPT TO:<${recipient}>${this.#getDsnRcptToArgs()}`);
        };
    ...
    #actionRCPT(str, callback) {
        ...
        if (!str.startsWith('2')) {
            ...
            err = this.#formatError(message, 'EENVELOPE', str, 'RCPT TO'); // 为失败的收件人存储错误
        ...
        if (!rcptQueue.length && !_recipientQueue.length) {
            if (rejected.length < to.length)
                _responseActions.push(str => this.#actionDATA(str, callback)), this.#sendCommand('DATA');
            else {
                err = this.#formatError("无法发送邮件 - 所有收件人都被拒绝", 'EENVELOPE', str, 'RCPT TO');
            ...
            _responseActions.push(str => this.#actionRCPT(str, callback));
            this.#sendCommand(`RCPT TO:<${curRecipient}>${this.#getDsnRcptToArgs()}`);
        }
    }
     ...
     * @param {String} str 来自服务器的消息
     */
    #actionDATA(str, callback) {
        // 如果不是23，则数据命令失败
        if (!regexs.DATA_RESPONSE.test(str)) return callback(this.#formatError('数据命令失败', 'EENVELOPE', str, 'DATA'));
     ...
     * @param {String} str 来自服务器的消息
     */
    #actionSMTPStream(str, callback) {
        return str.startsWith('2') ? callback(null, str) : callback(this.#formatError('消息失败', 'EMESSAGE', str, 'DATA'));
    }
     ...
     * @param {String} str 来自服务器的消息
     */
    #actionLMTPStream(recipient, final, str, callback) {
        // 检查响应状态码
        if (!str.startsWith('2')) {
            const { accepted } = this._envelope, index = accepted.indexOf(recipient),
                err = this.#formatError(`收件人 ${recipient} 的消息失败`, 'EMESSAGE', str, 'DATA');
    ...
    // 处理_XOAUTH2认证
    #actionAUTH_XOAUTH2_TOKEN(isRetry, callback) {
        ...
        oauth2.getToken(isRetry, (err, accessToken) => {
            if (err) {
                this.logger.info({ tnx: 'smtp', username: user, action: 'authfail', method: this._authMethod },
                    '用户 %s 认证失败', JSON.stringify(user));
                return callback(this.#formatError(err, 'EAUTH', false, 'AUTH XOAUTH2'));
            }
            this._responseActions.push(str => this.#actionAUTH_PLAIN_RESPONSE(str, isRetry, callback));
            this.#sendCommand(
