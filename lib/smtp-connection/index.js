...
// 导入依赖包
const TLS = require('tls'), { EventEmitter } = require('events'), { PassThrough } = require('stream'),
    { randomBytes, createHmac } = require('crypto'), DataStream = require('./data-stream'),
    { PK, OS, NET, regexs, resetRegex, getLogger, resolveHostname, callbackPromise, encodeXText, cleanup } = require('../shared'),

    // 默认超时(连接,套接字无活动,等待SMTP问候,DNS解析)时间（毫秒）
    CONNECTION_TIMEOUT = 2 * 60 * 1000, SOCKET_TIMEOUT = 10 * 60 * 1000, GREETING_TIMEOUT = 30 * 1000, DNS_TIMEOUT = 30 * 1000;
