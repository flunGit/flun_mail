'use strict';
// 引入必要的模块
const { PassThrough } = require('stream'), { randomBytes } = require('crypto'),

    // 引入自定义模块
    { QpEncoder } = require('../qp'), { Base64Encoder } = require('../base64'), { toASCII } = require('../punycode'),
    addressparser = require('../addressparser'),
    { fs, nmfetch, regexs, resetRegex, callbackPromise, cleanup, resolveStream } = require('../shared'),
    { detectMimeType, isPlainText, hasLongerLines, foldLines, parseHeaderValue, buildHeaderValue, encodeWord, encodeWords }
        = require('../mime-funcs'),

    // 引入换行符处理模块
    LeWindows = require('./le-windows'), LeUnix = require('./le-unix'), LastNewline = require('./last-newline'),

    B64 = 'base64', QP = 'quoted-printable';
...
class MimeNode {
    ...
    #processHeadersOp(key, value, singleOp, isSet) {
        ...
        key = this.#normalizeHeaderKey(key);                                       // 单个头部操作
     ...
     */
    setHeader(key, value) {
        // 如果未处理批量操作，说明是单个操作，已经在#processHeadersOp中处理完成
        if (!this.#processHeadersOp(key, value, this.setHeader, true)); // true表示是set操作
        return this;
    }
    ...
    addHeader(key, value) {
        // 如果未处理批量操作，说明是单个操作，已经在#processHeadersOp中处理完成
        if (!this.#processHeadersOp(key, value, this.addHeader, false)); // false表示是add操作
        return this;
    }
    ...
    setContent(content) {
        ...
        if (typeof this.content.pipe === 'function') {
            ...
            this.contentErrorHandler = err => {
                this.content.removeListener('error', this.contentErrorHandler), this.content = err;
            };
            this.content.once('error', this.contentErrorHandler);
            ...
            if (!encoding || ![B64, QP].includes(encoding)) {
                if (regexs.TEXT_TYPE.test(contentType)) {
                    if (this._isPlainText && !this._hasLongLines) encoding = '7bit';  // 如果没有特殊符号，则无需修改文本
                        ...
                        encoding = this.#getTextEncoding(content) === 'Q' ? QP : B64;
        ...
        this._headers.forEach(header => {
            ...
            switch (key) {
                case C_DISPOSITION:
                    ...
                    break;
                case C_TYPE:
                    structured = parseHeaderValue(value), this.#handleContentType(structured);
                    ...
                    if (filename) {
                        ...
                        param = this.#encodeWords(filename);
            ...
            value = this.encodeHeaderValue(key, value);
    ...
    stream(outputStream, options, done) {
        ...
        const sendContent = () => {
            ...
            if (content) {
                ...
                if (typeof content.pipe === 'function') {
                    content.removeListener('error', this.contentErrorHandler);
                    this.contentErrorHandler = err => callback(err), content.once('error', this.contentErrorHandler);
                }
                ...
                const createStream = () => {
                    if ([QP, B64].includes(transferEncoding)) {
                        ...
                        localStream = this.#getStream(content), localStream.pipe(contentStream);
                    } else {
                        // 任何不是QP或Base54的内容都按原样传递
                        localStream = this.#getStream(content), localStream.pipe(outputStream, { end: false });
                ...
                if (content._resolve) {
                    // 将流数据读取到Buffer中
                    resolveStream(this.#getStream(content), (err, resolvedValue) => {
                        if (err) return callback(err);
        ...
        if (this._raw) {
            setImmediate(() => {
                ...
                if (typeof this._raw.pipe === 'function') this._raw.removeListener('error', this.contentErrorHandler);

                const raw = this.#getStream(this._raw);
    ...
    setEnvelope(envelope) {
        ...
        if (envelope.from) {
            let list = [];
            this.#convertAddresses(this.parseAddresses(envelope.from), list);
        ...
        ['to', 'cc', 'bcc'].forEach(key => {
            if (envelope[key]) this.#convertAddresses(this.parseAddresses(envelope[key]), to); // 如果存在，则将地址添加到信封中
        });
    ...
    getAddresses() {
        ...
        this._headers.forEach(header => {
            ...
            if (['from', 'sender', 'reply-to', 'to', 'cc', 'bcc'].includes(key)) {
                ...
                this.#convertAddresses(this.parseAddresses(header.value), addresses[key]);
            }
        });
    ...
    getEnvelope() {
        ...
        this._headers.forEach(header => {
            ...
            if (key === 'From' || (!from && ['Reply-To', 'Sender'].includes(key))) {
                this.#convertAddresses(this.parseAddresses(value), list);
                if (list.length && list[0]) envelope.from = list[0].address;
            }
            else if (['To', 'Cc', 'Bcc'].includes(key)) this.#convertAddresses(this.parseAddresses(value), to);
        });
     ...
     */
    messageId() {
        return this.getHeader('Message-ID') || this.setHeader('Message-ID', this.#generateMessageId());
    }
    ...
    setRaw(raw) {
        ...
        if (this._raw && typeof this._raw.pipe === 'function') {
            // 预流处理程序。如果流被设置为内容，并且在此流执行任何操作之前触发'error'，则可能被触发
            this.contentErrorHandler = err => {
                this._raw.removeListener('error', this.contentErrorHandler), this._raw = err;
            };
            this._raw.once('error', this.contentErrorHandler);
        }
    ...
    parseAddresses(addresses) {
        return [].concat.apply([], [].concat(addresses).map(mAddress => {
            ...
            if (mAddress?.address) {
                ...
                mAddress.address = this.#normalizeAddress(address), mAddress.name = name;
                return [mAddress];
            }
     ...
     */
    #handleContentType(structured) {
        this.contentType = structured.value.trim().toLowerCase();
        this.multipart = regexs.MULTIPART.test(this.contentType) && this.contentType.split('/')[1];
        this.boundary = this.multipart ? structured.params.boundary = this.boundary || this.#generateBoundary() : false;
    }
    ...
    encodeHeaderValue(key, value = '') {
        ...
        switch (key) {
            ...
            case 'Cc':
            case 'Bcc':
            case 'Reply-To': return this.#convertAddresses(this.parseAddresses(value));
            // 用<>括起来的值
            case 'Message-ID':
            case 'In-Reply-To':
            case 'Content-Id':
                return ensureAngleBrackets(this.#encodeWords(cleanValue(value)));
            // 用<>括起来的值，以空格分隔的列表
            case 'References':
                value = [].concat(value)
                ...
                return this.#encodeWords(value.join(' ').trim());
            case 'Date':
                if (value instanceof Date) return value.toUTCString().replace('GMT', '+0000');
                return this.#encodeWords(cleanValue(value));
            case 'Content-Type':
            case 'Content-Disposition': return cleanValue(value); // 如果包含文件名，则已编码
            default: return this.#encodeWords(cleanValue(value)); // 默认encodeWords仅在需要时编码，否则返回原始字符串
        }
    }
    ...
    #convertAddresses(addresses = [], uniqueList = []) {
        ...
        [].concat(addresses).forEach(addrObj => {
            ...
            if (address) {
                const normAddr = this.#normalizeAddress(address);
                if (!name) values.push(normAddr.includes(' ') ? `<${normAddr}>` : normAddr);
                else if (name) values.push(`${this.#encodeAddressName(name)} <${normAddr}>`);

                if (!uniqueList.some(a => this.#normalizeAddress(a.address) === normAddr)) uniqueList.push(addrObj);
            }
            else if (group) {
                const groupListAddresses = (group.length ? this.#convertAddresses(group, uniqueList) : '').trim();
                values.push(`${this.#encodeAddressName(name)}:${groupListAddresses};`);
            }
        });
     ...
     */
        ...
        if (!rRUS.test(name)) {
            resetRegex(rQP);
            return rPA.test(name) ? `"${name.replace(rQP, '\\$1')}"` : encodeWord(name, this.#getTextEncoding(name), 52);
        }
        return name;
    }
     ...
     */
    #encodeWords(value) {
        return encodeWords(value, this.#getTextEncoding(value), 52, true);
    }
