...
import { MIME_TYPES, EXTENSIONS } from './mime-types.js';
import { regexs, resetRegex } from '../regexs.js';
import { base64Encode } from '../base64.js';
import { qpEncode } from '../qp.js';

// 默认的MIME类型(通用的二进制数据流) 和 默认的文件扩展名
const defaultMimeType = 'application/octet-stream', defaultExtension = 'bin',
     ...
     */
    _splitMimeEncodedString = (str = '', maxlen) => {
     ...
     */
    _buildHeaderParam = (key, data = '', maxLength = 50) => {
        const list = [];
     ...
     */
    encodeWords = (value, mimeWordEncoding = "Q", maxLength = 0, encodeAll) => {
    ...
    detectMimeType = filename => {
        ...
        const { ext, name = '' } = parse(filename),     // 解析文件路径获取扩展名
     ...
     */
    foldLines = (str = '', lineLength = 76, afterSpace) => {
        // 计算调整长度
