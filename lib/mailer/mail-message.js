...
import { regexs, resetRegex, resolveContent } from '../shared.js';
import { detectExtension, detectMimeType, isPlainText, encodeWord } from '../mime-funcs/index.js';

/**
 * MailMessage 类 - 负责构建和处理邮件消息对象
 *
 * @class MailMessage
 * @description 该类封装了邮件的完整构建流程,包括邮件数据的初始化、内容解析、
 *  头部规范化、地址处理、附件处理以及各类邮件头部的自动设置;
 *  通常由 Mailer 实例内部创建和调用,不建议直接实例化;
 */
class MailMessage {
    // 构造函数，初始化邮件消息对象
    constructor(mailer, data = {}) {
        ...
        this.data = this.#buildMailData(data, _defaults, options); // 初始化所有数据
    }

    #buildMailData(data, defaults, options) {
    ...
    resolveAll(callback) {
        ...
        addressKeys.forEach(address => {
            ...
            // 如果已有消息，从消息头解析地址
            if (this.message) value = [].concat(mimeNode.parseAddresses(rAddress) || []);
            // 否则如果数据中有，则直接从数据中解析
            else if (this.data[address]) value = [].concat(mimeNode.parseAddresses(this.data[address]) || []);
    ...
    normalize(callback) {
        ...
        this.resolveAll((err, data) => {
            ...
            Object.keys(headers).forEach(key => {
                ...
                if (value != null) {
                    ...
                    if (['references', 'in-reply-to', 'message-id', 'content-id'].includes(key))
                        value = this.message.encodeHeaderValue(key, value);
            ...
            if (list && typeof list === 'object') {
                const listHeaders = this.#getListHeaders(list);
            ...
            // 处理引用头部和回复头部
            if (references) normalizedHeaders.references = this.message.encodeHeaderValue('references', references);
            if (inReplyTo) normalizedHeaders['in-reply-to'] = this.message.encodeHeaderValue('in-reply-to', inReplyTo);
                ...
                value: [{
                    ...
                    value: [].concat(values)
                        ...
                        .map(val => {
                            ...
                            const formattedUrl = this.#formatListUrl(val.url); // 格式化URL
