'use strict';
// 引入所需模块
const EventEmitter = require('events'), { randomBytes } = require('crypto'), MailMessage = require('./mail-message'),
    MailComposer = require('../mail-composer'), DKIM = require('../dkim'), httpProxyClient = require('../http-proxy-client'),
    { detectExtension } = require('../mime-funcs'),
    { PK, util, NET, dns, regexs, resetRegex, getLogger, callbackPromise, newURL } = require('../shared');
...
class Mailer extends EventEmitter {
    constructor(transporter, options = {}, defaults = {}) {
        ...
        // 默认插件配置（编译和流处理阶段）
        this._defaultPlugins = { compile: [(...args) => this.#convertDataImages(...args)], stream: [] };

        this._userPlugins = { compile: [], stream: [] };     // 用户自定义插件
        this.meta = new Map();                               // 元数据存储
        const { dkim, proxy, component = 'mailer' } = this.options;
        ...
        ['close', 'isIdle', 'verify'].forEach(method => {
            this[method] = (...args) => {
                if (typeof this.transporter[method] === 'function') {
                    ...
                    if (method === 'verify' && typeof this.getSocket === 'function') this.#setupTransporterSocket();
    ...
    sendMail(data, callback = null) {
        ...
        if (typeof this.getSocket === 'function') this.#setupTransporterSocket(); // 设置传输器的getSocket方法

        // 记录发送日志
        const { name, version } = this.transporter;
        this.logger.debug({ tnx: 'transport', name, version, action: 'send' }, '使用 %s/%s 发送邮件', name, version);

        const isCallbackErr = (err, action, context, tnx = 'plugin') => {
            if (err) {
        ...
        // 处理编译阶段插件
        this.#processPlugins('compile', mail, err => {
            isCallbackErr(err, 'compile', '插件编译错误: %s'), mail.message = new MailComposer(mail.data).compile();
            mail.setMailerHeader(), mail.setPriorityHeaders(), mail.setListHeaders(); // 设置各种邮件头

            // 处理流阶段插件
            this.#processPlugins('stream', mail, err => {
                isCallbackErr(err, 'stream', '插件流错误: %s');
                ...
                this.transporter.send(mail, (err, ...args) => {
                    isCallbackErr(err, 'send', '发送错误: %s', 'transport'), callback(null, ...args);
                });
            });
        });
     ...
     */
    #processPlugins(step = '', mail, callback) {
        step = step.toString();
     ...
     * @param {Function} callback 回调函数
     */
    #convertDataImages(mail, callback) {
