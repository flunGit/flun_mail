...
import { detectExtension } from '../mime-funcs/index.js';
import { PK, format, isIP, dnsResolve, regexs, resetRegex, getLogger, callbackPromise, newURL } from '../shared.js';
...
class Mailer extends EventEmitter {
    ...
    getVersionString() {
        const { name, version } = this.transporter;
        return format('%s (%s; +%s; %s/%s)', PK.name, PK.version, PK.homepage, name, version);
    }
        ...
        this.getSocket = (options, callback) => {
            ...
            switch (protocol) {
                ...
                case 'socks4a': {
                    ...
                    // 如果代理主机是IP地址，则直接连接
                    if (isIP(hostname)) return connect(hostname);

                    return dnsResolve(hostname, (err, address) => {
