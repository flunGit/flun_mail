...
class Tokenizer {
     ...
     */
    #processChar(chr, nextChr) {
        const { escaped, operatorExpecting: oExpecting, operators } = this;
        // 检查是否是期望的结束操作符
        if (!escaped && chr === oExpecting) return this.#addOperatorToken(chr, nextChr);

        // 检查是否是新的开始操作符
        else if (!escaped && !oExpecting && chr in operators) return this.#addOperatorToken(chr, nextChr, operators[chr]);

        // 检查转义字符（在引号或注释中）
        else if (!escaped && ['"', "'"].includes(oExpecting) && chr === '\\') return this.escaped = true;
        this.#addTextChar(chr), this.escaped = false;  // 处理文本字符并重置转义标志
    }
     ...
     */
    #addOperatorToken(chr, nextChr, newExpecting = '') {
 ...
 */
const _replaceTextWithComment = data => {
    ...
    _extractEmailFromText = data => {
        ...
        for (let i = text.length - 1; i >= 0; i--) {
            ...
            if (!foundAddress) {
                // ✅ 移除 .trim(),保留原始空格,后续 join 会自然合并
                text[i] = currentText.replace(regexs.EMAIL_LOOSE, regexHandler);
                if (foundAddress) break;
            }
        }
        // 清理文本数组中可能产生的多余空白项
        data.text = data.text.filter(t => t.trim() !== '');
    },
    ...
    _handleAddress = tokens => {
        ...
        if (isGroup) return [{ name: text.join(' ') || '', group: group.length ? addressparser(group.join(',')) : [] }];
            ...
            const addressText = newTxt.join(' ').trim(), addressValue = newAdd.join(' ').trim(),// 合并文本和地址数据
                // 创建地址对象
                addressObj = { address: addressValue || addressText || '', name: addressText || addressValue || '' };

            // 如果地址和名称相同，根据内容决定清除哪个(如果是邮件地址,清除名称,否则,清除地址)
            if (addressObj.address === addressObj.name)
                addressObj.address.includes('@') ? addressObj.name = '' : addressObj.address = '';
