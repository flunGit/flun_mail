 ...
 */
const fs = require('fs'), path = require('path'), { PassThrough } = require('stream'), { randomBytes } = require('crypto'),
    MessageParser = require('./message-parser'), // 用于解析邮件头和正文
    RelaxedBody = require('./relaxed-body'),     // 用于宽松规范化邮件正文
    signer = require('./sign'),                  // 用于生成DKIM签名字段

    DKIM_ALGO = 'sha256',                // DKIM签名的默认哈希算法
    MAX_MESSAGE_SIZE = 2 * 1024 * 1024;  // 将大于此大小的消息缓冲到磁盘
