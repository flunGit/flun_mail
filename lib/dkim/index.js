...
import { MessageParser } from './message-parser.js';          // 用于解析邮件头和正文
import { RelaxedBody } from './relaxed-body.js';              // 用于宽松规范化邮件正文
import { generateDKIMSignature as signer } from './sign.js';  // 用于生成DKIM签名字段

// DKIM签名的默认哈希算法,最大保存消息;
const DKIM_ALGO = 'sha256', MAX_MESSAGE_SIZE = 2 * 1024 * 1024;

/**
 * DKIMSigner 类 - 实现 DKIM 签名的流式处理
 *
 * @class DKIMSigner
 * @description 该类负责在邮件传输过程中对邮件内容进行 DKIM 签名;
 * 它通过管道流的方式读取原始邮件数据,解析邮件头和邮件体,
 * 计算邮件体的哈希值,并使用提供的私钥生成 DKIM 签名头,
 * 最终将签名后的邮件数据写入输出流,支持多密钥签名、缓存
 * 溢出处理及宽松正文规范化;
 */
class DKIMSigner {
    // 初始化 DKIM 签名器实例
    constructor(options = {}, keys, input, output) {
        ...
        this.cachePath =
            this.cacheDir ? pJoin(this.cacheDir, `message.${Date.now()}-${randomBytes(14).toString('hex')}`) : false;
    ...
    cleanup() {
        ...
        unlink(this.cachePath, () => false);     // 删除缓存文件(忽略错误)
    }

    // 创建读取缓存流
    createReadCache() {
        // 将剩余数据管道传输到缓存文件
        this.cache = createReadStream(this.cachePath);
    ...
    // 创建写入缓存流
    createWriteCache() {
        this.output.usingCache = true, this.cache = createWriteStream(this.cachePath);  // 将剩余数据管道传输到缓存文件
        // 处理缓存流的错误和关闭事件
        this.cache.once('error', err => {
...
/**
 * DKIM 类 - DomainKeys Identified Mail 签名处理类
 *
 * @class DKIM
 * @description 用于对邮件内容进行 DKIM 签名。支持从配置中加载多个域名密钥，
 *              将字符串、Buffer 或流形式的邮件输入转换为经过签名的输出流。
 *              签名过程通过内部 DKIMSigner 完成，通常与邮件发送流程集成使用。
 *
 * @example
 *   // 1. 创建DKIM实例
 *   let dkim = new DKIM({
