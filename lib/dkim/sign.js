// 引入必要的模块
import { createSign } from 'crypto';                // Node.js加密模块，用于生成签名
import { toASCII } from '../punycode.js';           // 用于处理Unicode域名的punycode转换
import { foldLines } from '../mime-funcs/index.js'; // 提供MIME相关工具函数
 ...
 */
const _generateDKIMHeader = (domainName, keySelector, fieldNames, hashAlgo, bodyHash) => {
    // 构建DKIM签名头的各个部分
    let dkim = [
        'v=1',                      // 版本
        `a=rsa-${hashAlgo}`,        // 算法
     ...
     */
    _relaxedHeaders = (headers, fieldNames = '', skipFields = '') => {
     ...
     */
    generateDKIMSignature = (headers, hashAlgo, bodyHash, options = {}) => {
