import { MimeNode } from './mime-node/index.js';
import { regexs, resetRegex, parseDataURI } from './shared.js';
import { detectMimeType, detectExtension } from './mime-funcs/index.js';
/**
...
class MailComposer {
    ...
    compile() {
        ...
        else if (this._useMixed) this.message = this.#createMixed();                     // 混合类型：内容 + 多个附件
        else if (this._useAlternative) this.message = this.#createAlternative();         // 替代类型：纯文本 + HTML
        else if (this._useRelated) this.message = this.#createRelated();                 // 相关类型：HTML + 内联资源
        else this.message = this.#createContentNode(false, [aTives, attached].flatMap(arr => arr || [])
    ...
    getAttachments(findRelated = false) {
        ...
        const pAttachments = attachments.map((attachment, i) => {
            const pAtt = this.#prepareAttachmentObject(attachment), // 预处理附件
    ...
    getAlternatives() {
        ...
        const gAlternatives = Array.prototype.concat.call([], text_, html_, watchHtml_, amp_, eventObject, alternatives)
            .filter(Boolean).map(originalAlternative => {
                const oAtive = this.#prepareAttachmentObject(originalAlternative), // 预处理附件
     ...
     */
    #prepareAttachmentObject(obj) {
        // 处理dataURL格式的内容和标准化路径
        const prepared = regexs.DATA_URL.test(obj.path || obj.href) ? { ...obj, ...this.#processDataUrl(obj) } : { ...obj },
            { path } = prepared;
     ...
     */
    #createMimeNode(parentNode, mimeType) {
        return !parentNode ? this.#createRootMimeNode(mimeType) : parentNode.createChild(mimeType, this.#getChildNodeOptions());
    }
     ...
     */
    #createMixed(parentNode) {
        const node = this.#createMimeNode(parentNode, 'multipart/mixed');
        // 根据内容类型添加子节点
        if (this._useAlternative) this.#createAlternative(node);
        else if (this._useRelated) this.#createRelated(node);

        // 添加所有替代内容和附件
        [...(this._useAlternative ? [] : this._alternatives || []), ...(this._attachments.attached || [])]
            .filter(element => !this._useRelated || element !== this._htmlNode)
            .forEach(element => this.#createContentNode(node, element));
     ...
     */
    #createAlternative(parentNode) {
        const node = this.#createMimeNode(parentNode, 'multipart/alternative');
        // 添加所有替代内容
        this._alternatives.forEach(ative => {
            this._useRelated && this._htmlNode === ative ? this.#createRelated(node) : this.#createContentNode(node, ative);
        });
     ...
     */
    #createRelated(parentNode) {
        const node = this.#createMimeNode(parentNode, 'multipart/related; type="text/html"');
        // 添加HTML内容和相关附件
        this.#createContentNode(node, this._htmlNode);
        this._attachments.related.forEach(alternative => this.#createContentNode(node, alternative));
        return node;
    }
     ...
     */
    #createContentNode(parentNode, element = {}) {
     ...
     */
    #createRootMimeNode(contentType) {
