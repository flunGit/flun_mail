...
import { nmfetch, getLogger, resolveStream } from './shared.js';
 ...
 *     * accessToken 是一个字符串
 *     * expires 是可选的过期时间（毫秒）
 * 如果使用了 provisionCallback，则 flun-mail 不会尝试自行生成令牌
 *
 * @constructor
...
class XOAuth2 extends Stream {
    constructor(options = {}, logger) {
        ...
        const ttlMs = (Number(timeout) || 0) * 1000;
        // 如果指定了过期时间,则使用它,否则使用 TTL,否则,如果指定了 TTL,则使用它,否则使用默认值
        this.expires = Number(expires) || (ttlMs ? Date.now() + ttlMs : 0);
    }

    /**
     * 返回或生成（如果之前的已过期）XOAuth2 令牌
     *
     * @param {Boolean} renew 如果为 false 则使用缓存的访问令牌（如果可用）
     * @param {Function} callback 带有错误对象和令牌字符串的回调函数
     */
    getToken(renew, callback) {
        const { user, refreshToken, serviceClient } = this.options, { accessToken, expires, provisionCallback, renewing } = this,
            // 重用令牌
            reuseToken = message => {
        ...
        // 如果不需要续订且令牌有效，则直接重用
        if (!renew && accessToken && (!expires || expires > Date.now())) return reuseToken('为 %s 重用现有访问令牌');
    ...
    generateToken(callback) {
        ...
        if (serviceClient) {
            ...
            try {
                ...
                return callback(new Error(`JWT签名失败: ${err.message}`));
            }
            ...
            loggedUrlOptions = {
                ...
                refresh_token: `${refreshToken.slice(0, 6)}...`, grant_type
            };
        }
